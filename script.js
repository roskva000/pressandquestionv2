...
function calculateCadence() {
    // Returns average interval in ms over the last 10 clicks
    if (clickTimestamps.length < 2) return 1000; // Default 1 sec
    const recent = clickTimestamps.slice(-10);
    let totalInterval = 0;
...
function getAtmosphereForPath(path) {
    const moods = {
        'chaos': 'mood-tension',
        'pain': 'mood-tension',
        'compulsion': 'mood-intense',
        'time': 'mood-intense',
        'silence': 'mood-ethereal',
        'dream': 'mood-ethereal',
        'existence': 'mood-ethereal',
        'acceptance': 'mood-ethereal',
        'ego': 'mood-cold',
        'control': 'mood-cold',
        'simulation': 'mood-cold',
        'purpose': 'mood-cold',
        'memory': 'mood-cold' // Defaulting memory to cold/neutral
    };
    return moods[path] || '';
}

function setAtmosphere(path) {
    // Remove existing mood classes
    document.body.classList.remove('mood-tension', 'mood-ethereal', 'mood-cold', 'mood-intense');

    const moodClass = getAtmosphereForPath(path);
    if (moodClass) {
        // Simple visual transition handle via CSS
        document.body.classList.add(moodClass);
    }
}

function selectDynamicPath() {
    if (typeof narrativePaths === 'undefined') return 'curiosity';

    const cadence = calculateCadence(); // ms per click
    const restrictedKeys = ['corruption', 'corruptionRecovery', 'end'];

    // Classify Current State
    let state = 'neutral';
    if (cadence < 400) state = 'agitated'; // > 2.5 clicks/sec
    if (cadence > 1500) state = 'contemplative'; // < 0.6 clicks/sec

    // Theme Pools
    const agitatedThemes = ['chaos', 'pain', 'ego', 'control', 'compulsion', 'time'];
    const contemplativeThemes = ['silence', 'existence', 'acceptance', 'dream', 'purpose'];

    // Filter available paths (exclude restricted and already used if possible to avoid immediate repeat)
    // Note: We allow repeat if pool is exhausted, handled by existing logic style but refined.
    const allPaths = Object.keys(narrativePaths).filter(key =>
        !key.startsWith('early_') && !restrictedKeys.includes(key)
    );
    ...
    if (state === 'agitated') {
        // 70% chance Agitated, 30% Random
        if (Math.random() < 0.7) {
            pool = allPaths.filter(p => agitatedThemes.includes(p));
        }
    } else if (state === 'contemplative') {
        // 70% chance Contemplative, 30% Random
        if (Math.random() < 0.7) {
            pool = allPaths.filter(p => contemplativeThemes.includes(p));
        }
    }

    // Fallback to all paths if pool is empty (or purely random choice)
    if (pool.length === 0) pool = allPaths;

    // Filter used paths from the selected pool to ensure variety
    let availableInPool = pool.filter(p => !usedPaths.includes(p));

    // If pool exhausted in this category, reset usedPaths slightly or pick from pool anyway
    if (availableInPool.length === 0) {
        // Soft reset: we don't want to clear ALL used paths, just maybe allow re-picking from this pool
        availableInPool = pool;
        // If we really exhausted everything, clear global usedPaths (rare)
        if (usedPaths.length >= allPaths.length - 2) usedPaths = [];
    }

    const selectedPath = availableInPool[Math.floor(Math.random() * availableInPool.length)];

    // Track usage
    usedPaths.push(selectedPath);
    if (usedPaths.length > 5) usedPaths.shift(); // Keep memory short to allow revisiting themes eventually
...
function selectRandomPath() {
    ...
    return selectDynamicPath();
}

function switchToNextPath() {
    currentPath = selectDynamicPath();
    currentMessageIndex = 0;
    setAtmosphere(currentPath); // Trigger atmospheric shift
}
